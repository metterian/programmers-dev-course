...
앞서, 상자에서 과일을 꺼내는 예시에서 베이지안 정리를 통해 관측값들을 이용, 사전확률을 사후확률로 바꾸는 역할을 했습니다. (빨간 상자에서 과일을 꺼냈을 때 그 과일이 오렌지일 확률 $\Rightarrow$ 오렌지를 꺼낸 후(사후확률), 오렌지를 꺼낸 상자(사전확률)를 추론 하는 것)

다음 식을 통해, 다항식 곡선 피팅 문제에도 가중치에 대한 불확실성을 표현 할 수 있습니다. 다음 식의 의미는 $D$ 를 관측한 후의 **$\mathbf{w}$에 대한 불확실성**을 사후확률 $p(\mathbf{w} \mid D)$ 로 표현 한 것입니다.
$$
...
위의 식을 통해 실제 데이터(새로운 관측)을 통해 예측된 $\mathbf{w}$의 확률(likelihood)를 조합하여 실제 $\mathbf{w}$의 사후 확률(posterior)를 기술 하고 있습니다. 위 식을 이용해서 다음의 식을 만들어 낼 수 있습니다. 
$$
